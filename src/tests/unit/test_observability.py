...
# ---------------------------------------------------------------------------
# HYBRID_PARTIAL_RATE — Sprint 7 Task 11
# ---------------------------------------------------------------------------

class TestHybridPartialRateMetric:
    """HYBRID_PARTIAL_RATE increments on partial-failure fallback modes."""
    ...
    def test_hybrid_partial_rate_exists_in_module(self):
        from src.observability.prometheus_metrics import HYBRID_PARTIAL_RATE
        assert HYBRID_PARTIAL_RATE is not None

    def test_hybrid_partial_rate_in_all(self):
        import src.observability.prometheus_metrics as m
        assert "HYBRID_PARTIAL_RATE" in m.__all__

    def test_rag_only_fallback_increments(self):
        from src.observability.prometheus_metrics import HYBRID_PARTIAL_RATE
        before = self._sample_value(
            "erp_rag_hybrid_partial_total", {"fallback_mode": "rag_only"}
        )
    ...
    def test_sql_only_fallback_increments(self):
        from src.observability.prometheus_metrics import HYBRID_PARTIAL_RATE
        before = self._sample_value(
            "erp_rag_hybrid_partial_total", {"fallback_mode": "sql_only"}
        )
        HYBRID_PARTIAL_RATE.labels(fallback_mode="sql_only").inc()
        after = self._sample_value(
            "erp_rag_hybrid_partial_total", {"fallback_mode": "sql_only"}
        )
        assert after - before == pytest.approx(1.0)

    def test_rag_only_and_sql_only_are_distinct_label_sets(self):
        from src.observability.prometheus_metrics import HYBRID_PARTIAL_RATE
        HYBRID_PARTIAL_RATE.labels(fallback_mode="rag_only").inc(5)
        HYBRID_PARTIAL_RATE.labels(fallback_mode="sql_only").inc(3)
        rag = self._sample_value(
            "erp_rag_hybrid_partial_total", {"fallback_mode": "rag_only"}
        )
        sql = self._sample_value(
            "erp_rag_hybrid_partial_total", {"fallback_mode": "sql_only"}
        )
        assert rag != sql
