...
def _flood_user(user_id: str, count: int) -> None:
    """Pre-populate the user counter to *count* hits within the window."""
    for _ in range(count):
        _user_counter.increment(user_id)


def _flood_ip(ip: str, count: int) -> None:
    """Pre-populate the IP counter to *count* hits within the window."""
    for _ in range(count):
        _ip_counter.increment(ip)
...
class TestUserRateLimit:
    ...
    def test_user_at_limit_still_passes(self):
        ...
        # pre-populate to exactly the limit
        _flood_user(uid, _USER_LIMIT_PER_MIN)
    ...
    def test_user_over_limit_returns_429(self):
        ...
        _flood_user(uid, _USER_LIMIT_PER_MIN + 5)
        client = TestClient(_make_app(), raise_server_exceptions=False)
        r = client.get("/health", headers={"x-user-id": uid})
        assert r.status_code == 429
    ...
    def test_user_429_response_body(self):
        ...
        _flood_user(uid, _USER_LIMIT_PER_MIN + 1)
        client = TestClient(_make_app(), raise_server_exceptions=False)
        r = client.get("/health", headers={"x-user-id": uid})
    ...
    def test_different_users_isolated(self):
        ...
        _flood_user(limited_uid, _USER_LIMIT_PER_MIN + 5)
        # clean_uid has no prior hits
        client = TestClient(_make_app(), raise_server_exceptions=False)
...
class TestIPRateLimit:
    ...
    def test_ip_over_limit_returns_429(self):
        ...
        ip = f"1.2.3.{int(time.monotonic()) % 200}"
        # Use a fresh IP string to avoid counter contamination across test runs
        unique_ip = f"192.168.{int(time.monotonic() * 1000) % 255}.1"
        _flood_ip(unique_ip, _IP_LIMIT_PER_MIN + 5)
        client = TestClient(_make_app(), raise_server_exceptions=False)
    ...
    def test_ip_429_response_body(self):
        ...
        _flood_ip(unique_ip, _IP_LIMIT_PER_MIN + 1)
        client = TestClient(_make_app(), raise_server_exceptions=False)
        r = client.get("/health", headers={"x-forwarded-for": unique_ip})
    ...
    def test_different_ips_isolated(self):
        ...
        _flood_ip(limited_ip, _IP_LIMIT_PER_MIN + 5)
        # Give clean_ip a fresh user-id so it doesn't collide on user counter
        client = TestClient(_make_app(), raise_server_exceptions=False)
        assert client.get(
            "/health",
